...
# Vercel's /var/task directory is read-only. Keep the project files on the
# normal filesystem everywhere else, but put runtime-generated storage in
# /tmp on Vercel. /tmp is writable for the lifetime of the serverless instance.
if os.environ.get("VERCEL") == "1":
    BASE_DIR = Path(os.environ.get("BOT_RUNTIME_DIR", "/tmp/warrior_bot_runtime"))
else:
...
if not TOKEN:
    raise RuntimeError(
        "BOT_TOKEN environment variable is required. Add BOT_TOKEN in Vercel Environment Variables."
    )
...
SUPPORT_USR = "@XKingCobra"
UPDATE_CH   = "https://t.me/+fzRIfGTNbycwMTVl"
...
def _cleanup_bot_files(b: Dict[str, Any], *, remove_record: bool = False) -> None:
    """Stop first, then safely remove this bot's sandbox, uploaded blobs and keys."""
    bot_id = str(b.get("_id", ""))
    stop_child(bot_id, manual=True)

    # Only remove a sandbox directory that is inside this application's sandbox root.
    try:
        raw_dir = str(b.get("dir") or "")
        if raw_dir:
            candidate = Path(raw_dir).resolve()
            root = DIRS["sandbox"].resolve()
            if candidate != root and root in candidate.parents:
                shutil.rmtree(candidate, ignore_errors=True)
    except Exception as exc:
        print(f"[delete] sandbox cleanup failed for {bot_id}: {exc}", file=sys.stderr)

    # Current uploads are a list of metadata dictionaries. Older backups may use a dict.
    entries = b.get("enc_files") or []
    ...
    if isinstance(entries, (list, tuple)):
        for item in entries:
            ...
            enc_path = str(item.get("enc_path") or item.get("path") or "")
            try:
                if enc_path:
                    candidate = Path(enc_path).resolve()
                    root = DIRS["encfiles"].resolve()
                    if candidate != root and root in candidate.parents:
                        candidate.unlink(missing_ok=True)
            except Exception as exc:
                print(f"[delete] encrypted blob cleanup failed: {exc}", file=sys.stderr)
            if key_id:
                try:
                    KEYRING.remove(key_id)
                except Exception as exc:
                    print(f"[delete] key cleanup failed: {exc}", file=sys.stderr)

    # Remove empty per-uploader directories, but never the encfiles root.
    try:
        for folder in DIRS["encfiles"].iterdir():
            if folder.is_dir():
                try:
                    folder.rmdir()
                except OSError:
    ...
    # Best effort remote cleanup; this also removes the GitHub metadata backup.
    try:
        _gh_delete_bot_files(b)
    except Exception as exc:
        print(f"[delete] GitHub cleanup failed for {bot_id}: {exc}", file=sys.stderr)
...
def action_bot_delete(call: types.CallbackQuery, bot_id: str) -> None:
    ...
    if not b or (b["owner"] != call.from_user.id and not is_admin(call.from_user.id)):
        ack(call, "Not yours"); return
    # This button is explicitly "Delete Record"; preserve files as its confirmation says.
    stop_child(bot_id, manual=True)
    delete_bot_doc(bot_id)
    audit(call.from_user.id, "bot_delete", f"bot={bot_id}")
    ack(call, "Deleted")
...
def action_bot_delfiles(call: types.CallbackQuery, bot_id: str) -> None:
    ...
    if not b or (b["owner"] != call.from_user.id and not is_admin(call.from_user.id)):
        ack(call, "Not yours"); return
    _cleanup_bot_files(b, remove_record=False)
    audit(call.from_user.id, "bot_delfiles", f"bot={bot_id}")
...
def action_bot_delall(call: types.CallbackQuery, bot_id: str) -> None:
    ...
    if not b or (b["owner"] != call.from_user.id and not is_admin(call.from_user.id)):
        ack(call, "Not yours"); return
    _cleanup_bot_files(b, remove_record=True)
    audit(call.from_user.id, "bot_delall", f"bot={bot_id}")
